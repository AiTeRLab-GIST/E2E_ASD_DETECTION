...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        batch, timestamp, n_feats = x.shape
        x = x.reshape(batch * timestamp,n_feats)
        x = self.dense1(x)
        z = self.dense2(x)
        x = self.dense3(z)
        x = self.proj(x)
        return x, z.reshape(batch, timestamp, -1)
...
class MultiTaskAutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dense3(z)
        x = self.proj(x)
        z = z.reshape(batch, timestamp, -1)
        aux = self.aux(z)
        aux = nn.MaxPool1d(aux.shape[1])(aux.transpose(2,1)).squeeze()
