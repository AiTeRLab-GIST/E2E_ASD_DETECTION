...
class DataProcessor:
    ...
    def save_db_list_to_csv(self, df):
        train_df, test_df = train_test_split(df, test_size=0.2, random_state=101, stratify=df[conf.outputs])

        train_df = train_df.reset_index(drop=True)
        test_df = test_df.reset_index(drop=True)

        train_df.to_csv(f"{conf.save_path}/{conf.df_names[0]}", sep="\t", encoding="utf-8", index=False)
        test_df.to_csv(f"{conf.save_path}/{conf.df_names[1]}", sep="\t", encoding="utf-8", index=False)
